...
const ProductUpdatePage = () => {
  ...
  const handleFetchProductDetail = useMutation({
  ...
  const fetchImageProduct = useMutation({
  ...
  useEffect(() => {
    const fetchData = async () => {
      const data = await handleFetchProductDetail.mutateAsync({
        DCMNCODE: "INPPRODUCT",
        KEY_CODE: id,
      });
      await fetchImageProduct.mutateAsync(data[0].DCMNFILE[0].FILE_URL);
    };
                  ...
                  <div className="grid w-full items-center gap-1.5 h-full">
                    <Label htmlFor="picture" className="h-full">
                      <div className="bg-slate-50 w-full h-full border border-gray-200 shadow-sm overflow-hidden">
                        <img
                          src={
                            fetchImageProduct.data
                              ? URL.createObjectURL(fetchImageProduct.data)
                              : image != null
